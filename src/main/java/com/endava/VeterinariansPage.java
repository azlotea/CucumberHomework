...
public class VeterinariansPage {
    ...
    private List<WebElement> editVetBtnList;
    ...
    public VeterinariansPage(WebDriver webDriver) {
        ...
        editVetBtnList = webDriver.findElements(By.xpath("//tbody//tr//td[3]//button[1]"));
    ...
    public void editNewVet(String nameVet) {
        for (int i = 0; i < veterinariansList.size(); i++) {
            String nameNewVet = veterinariansList.get(i).getText();
            if (nameNewVet.equals(nameVet)) {
                editVetBtnList.get(i).click();
                break;
            }
        }
    }
